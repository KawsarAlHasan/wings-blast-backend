...
exports.createBonus = async (req, res) => {
  try {
    ...
    if (!bonus_type || !name || !code) {
      return res.status(400).send({
        success: false,
        message: "Please provide bonus_type, name & code field",
      });
    }

    const [data] = await db.query("SELECT * FROM bonus WHERE code=? ", [code]);

    if (data.length == 1) {
      return res.status(201).send({
        success: false,
        message: "This code already exists. Please provide new code",
      });
    }

    // Insert bonus into the database
    const [result] = await db.query(
      "INSERT INTO bonus ( bonus_type, name, code, date, discount_percentage, discount_amount, start_date, end_date, is_date, is_discount_percentage ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)",
      [
        bonus_type,
        name || "",
        code || "",
    ...
    if (result.insertId == 0) {
      return res.status(500).send({
        success: false,
        message: "Failed to insert Bonus, please try again",
      });
    }
    ...
    res.status(200).send({
      success: true,
      message: "Bonus inserted successfully",
    });
  } catch (error) {
    res.status(500).send({
      success: false,
      message: "An error occurred while inserting the Bonus",
      error: error.message,
    });
  }
};
...
exports.getAllBonus = async (req, res) => {
  try {
    ...
    const [data] = await db.query("SELECT * FROM bonus WHERE bonus_type =?", [
      bonus_type,
    ]);

    res.status(200).send({
      success: true,
      message: `Get all ${bonus_type}`,
      data,
    });
  } catch (error) {
    res.status(500).send({
