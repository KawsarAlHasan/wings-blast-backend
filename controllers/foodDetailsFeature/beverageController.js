...
exports.getAllBeverage = async (req, res) => {
  try {
    ...
    let query = "SELECT * FROM beverage";
    const params = [];
...
exports.updateBeverageStatus = async (req, res) => {
  try {
    const { id } = req.params;
    ...
    // Execute the update query
    const [result] = await db.query(
      "UPDATE beverage SET status = ? WHERE id = ?",
      [status, id]
    );
    ...
    res.status(200).send({
      success: true,
      message: "beverage status updated successfully",
    });
  } catch (error) {
    res.status(500).send({
      success: false,
      message: "Error status updating beverage",
      error: error.message,
    });
  }
};
